...
async function aplicarFiltrosAvanzados() {
  ...
  if (filtrosAvanzados.especie)
    params.append('especie', filtrosAvanzados.especie);
  if (filtrosAvanzados.tamano) params.append('tamano', filtrosAvanzados.tamano);
  if (filtrosAvanzados.edad && filtrosAvanzados.edad > 0)
    params.append('edad', filtrosAvanzados.edad);
  ...
  if (contenedor)
    contenedor.innerHTML =
      '<div class="text-center mt-5"><div class="spinner-border text-primary"></div><p>Filtrando...</p></div>';

  try {
    // endpoint de búsqueda
    const response = await fetch(
      `${API_URL}/mascotas/buscar?${params.toString()}`,
      {
        method: 'GET',
        headers: getHeaders(),
      }
    ...
    console.error(error);
    if (contenedor)
      contenedor.innerHTML = `<p class="text-center text-danger mt-5">No se encontraron resultados.</p>`;
  }
}
...
function renderizarPublicaciones(lista) {
  ...
  contenedor.innerHTML = lista
    .map((pub) => {
      ...
      // Likes locales
      const likesGuardados = JSON.parse(
        localStorage.getItem('mis_likes') || '[]'
      );
      ...
      // Comentarios
      const listaComentarios = (pub.comentarios || [])
        .map(
          (c) => `
            <div class="mb-1 border-bottom pb-1">
                <strong>${c.autor ? c.autor.username : 'Usuario'}:</strong> ${
            c.texto
          }
            </div>
        `
...
function getTelefonoUsuarioPorMascotaId(petId) {
  const pub = publicaciones.find(
    (p) => p.mascota && p.mascota.id_mascotas === petId
  );
...
function handleAdoptionFormSubmit(event) {
  ...
  const nombreMascota = document
    .getElementById('nombreMascotaModal')
    .textContent.trim();
  const telefonoDueno = getTelefonoUsuarioPorMascotaId(petId);

  if (!telefonoDueno || telefonoDueno === 0) {
    alert(
      '⚠️ El dueño no ha registrado un teléfono. Envía la solicitud y le notificaremos por el sistema.'
    );
  ...
  const urlWhatsapp = `https://wa.me/${telefonoDueno}?text=${encodeURIComponent(
    mensaje
  )}`;
  ...
  document.querySelectorAll('.filter-link').forEach((link) => {
    link.addEventListener('click', (e) => {
      ...
      if (el.classList.contains('active')) {
        ...
        document
          .querySelectorAll(`.filter-link[data-tipo="${tipo}"]`)
  ...
  if (ageSlider && ageValueEl) {
    ...
    ageSlider.addEventListener('input', () => {
      ...
      ageValueEl.textContent =
        valor > 0 ? `Máx. ${valor} años` : 'Cualquier edad';
