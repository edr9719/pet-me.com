document.addEventListener('DOMContentLoaded', () => {
    ...
    button.addEventListener('click', (event) => {
      ...
      if (petCard) {
        ...
        const petName = petNameElement
          ...
          : 'una mascota';
        alert(
          `Has hecho clic en Contactar para ${petName}. ¡Esperamos que encuentres un buen compañero!`
        );
      } else {
        alert(
          'Has hecho clic en Contactar. ¡Esperamos que encuentres un buen compañero!'
        );
      }
    });
  });
  ...
  favoriteButtons.forEach((button) => {
    button.addEventListener('click', (event) => {
      ...
      console.log(
        `${petName} ha sido ${
          event.target.classList.contains('active-favorite')
            ? 'añadida a'
            : 'eliminada de'
        } favoritos.`
      );
...
function createPetCard(pet) {
  const cardHtml = `
        <div class="card pet-card mb-3">
            <img src="${pet.imageUrl}" class="card-img-top pet-img" alt="${
    pet.name
  }">
            <div class="card-body">
                ...
                <p class="card-text pet-details">${pet.age} · ${pet.gender} · ${
    pet.location
  }</p>
